...
Following \citet{} we assess the transfer learning performance of each $\mathcal{M}_S/\mathcal{M}_T$ pair by computing the area ratio metric $r$ for each learning curve as follows:
\begin{equation}
...
\end{equation}
and $a$ and $b$ correspond to the first and last training epochs respectively (see Fig. \ref{} for a visualization of $r$). Our results are presented in Tables \ref{} and \ref{}, where we report for all different $\mathcal{M}_S/\mathcal{M}_T$ pairs the performance obtained after fini-tuning a DQV pre-trained agent, and a DDQN pre-trained agent respectively.  
