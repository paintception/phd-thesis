...
\begin{table}
	\caption{Results obtained when classifying the bounding boxes of the three different MINERVA benchmarks ting with models that do not come as pre-trained on any sort of source task $\mathcal{T}_S$. We can see that the best performing architecture is , but that its performance is significantly worse than the one that is obtained when the same model comes as pre-trained (see Table \ref{table:minerva_tl_results}).}
...
\begin{table}
	\caption{Results obtained when classifying the bounding boxes of the three different MINERVA benchmarks after adapting transfer learning. We can again see that, compared to the results presented in Table \ref{table:minerva_no_tl_results}, this approach yields significant benefits. Furthermore, differently from what we observed in Chapter \ref{ch:tl_natural_to_non_natural}, we now see that the overall best performing architecture is InceptionV3 instead of ResNet50.}
\resizebox{\columnwidth}{!}{%
	\input{./Tables/Chapter05/classification_results.tex}%
}
