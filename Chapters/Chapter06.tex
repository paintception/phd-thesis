...
\end{equation}
where $p_{\tau}$ is $|\delta_\tau + \epsilon|$ with $\epsilon$ being a small positive number ensuring that the probability of sampling a trajectory remains positive even in the edge case where the TD-error is $0$. Although simple and intuitive implementing a PER buffer is not that straightforward and still presents some algorithmic caveats that need to be taken into account. Yet, if done correctly it dramatically improves the sample efficiency of Deep Q-Networks.

\paragraph{Dueling Networks:} while the DDQN algorithm directly tackles a fundamental algorithmic bias which characterizes the way DQN learns the $Q$ function, and PER addresses the inefficiency of its memory buffer, the contribution presented by \citet{wang2016dueling} is of slightly different nature. Their work in fact consists in a novel type of neural architecture, called the Dueling Network, which results in better DRL. We can see this as a contribution which is more similar to the kind of advancements that are made by the computer vision community, and that we reviewed in the previous chapter, rather than
it
. Nevertheless Wang's work is a perfect example which showcases how in DRL, carefully designing the function approximator is just as important as properly defining its objective function. A Dueling network is a network that after having performed a series of convolutions, instead of directly outputting the state-action values for a specific state as DQN and DDQN do, adds some intermediate steps. The idea is to also estimate the value of a stat together with the advantages for each action. The first one is given by Eq. \ref{eq:state_value_function}, while the latter defined as $A$ can simply be computed as follows:
\begin{equation}
	A^{\pi}(s,a) = Q^{\pi}(s,a) - V^{\pi}(s).
\end{equation}
In order to successfully estimate state values, advantages and state-action values the network requires three separate streams that are all initialized with their own parameters. The final $Q$ function of the model is given by combining what is learned by each stream and is defined as follows:
\begin{multline}
...
\end{multline}
which makes the Dueling architecture resemble models that are usually built for multitask learning. In the end the outputs of both streams get combined and the network finally outputs the different $Q$ values for all state-action pairs and the network is trained with the same objective function that is also used by DDQN.
The idea of taking into account the state-value function when learning the $Q$ function is a concept which will come back in this dissertation in chapters \ref{ch:dqv_family_of_algorithms} and \ref{ch:dqn_transfer}.
