...
In the previous chapter we have presented the Supervised Learning (SL) paradigm, a machine learning framework that aims at constructing models which can answer statistical questions about data coming in the form of input-output pairs. When these models are built successfully it is then possible to use them for making predictions about the behavior of new unseen data. Training SL models is a process which comes in a very static flavor. Datasets are divided into training, validation and testing sets, and besides providing a model with a large set of samples drawn from these datasets, there is no real interaction between the learning algorithm and the data that drives the learning process. In Reinforcement Learning (RL) this drastically changes. In this context the goal is not anymore that of learning a mapping between a set of fixed input samples and their respective targets, but to to train an algorithm such that it learns how to interact with its environment. This algorithm is usually denoted as the \textcolor{RoyalBlue}{agent} and it can come in numerous forms. For example it can be a self-driving car which needs to learn how to drive; or a recommendation system whose goal is to propose products to users navigating the web. More generally a RL agent is any kind of system that has to choose which action to perform when it is dealing with a certain situation. It is however not enough for an agent to simply learn how to interact with the environment, it is in fact also desirable that the behavior that is the result of the agent's actions is in line with what is broadly denoted as `intelligent". Going back to the previously introduced self-driving car example, an `intelligent" goal would be to train an agent such that it is not only able to drive autonomously, but that is also able to do this whilst complying with the driving code. Because of this concept of having to learn how to make (intelligent) decisions while interacting, RL problems are also sometimes denoted as optimal decision making problems, although it is up to the reader to decide whether acting optimally necessarily coincides with acting intelligently.   




Previous chapter input-output pairs coming in the form of a dataset, with the aim of constructing a model that captures the properties of the data which can then be used to predict the behavior of unseen new data
science of learning how to make decisions while interacting
goal, agent needs to learn how to interact with environment, some examples, large practical applications.
key component of time, we learn without examples of good behavior, no fixed dataset.
therfore the sl paradigm where we want to extract 
agents need to learn how to solve but also how to adapt, algorithms that learn in an online fashion. all these properties -> optimal decision making


\section{Markov Decision Processes}
\label{sec:mdps}

In order to develop and apply RL algorithms to optimal decision making problems, we need to formulate the problem in a specific framework: in RL this is done with Markov Decision Processes (MDPs) \cite{puterman1990markov,puterman2014markov}. Throughout this thesis we will characterize MDPs, and the resulting RL concepts, by using the mathematical notation that was used by \citet{sutton2018reinforcement} in their seminal book about RL, although it is worth noting that within the literature, different formulations can be found that express the same kind of concepts \cite{bertsekas1995neuro,busoniu2010reinforcement,bertsekas2000dynamic,bertsekas2019reinforcement}.
...
\begin{figure}[htb!]
	\centering
	\input{Images/Chapter06/rl_loop.tex}
...
\label{eq:mc_update}
\end{equation}
where $\alpha \in [0,1]$ is the learning rate controlling how much we want to change the value estimate of a state based on $G_t$. As a practical example let us consider the MDP represented in Fig. \ref{fig:mdp}. Let us assume that the starting state of the environment is $s_0$ while the terminal state of the environment is $s_2$, and that the agent follows a policy $\pi$ that results into the following state visits: $s_1, s_0, s_3$. The rewards associated to each visited state are therefore $-1, +2$ and $+3$ respectively. If we set the discount factor to $0.99$ we know that the real discounted return that is obtained at the end of the agent-environment interaction when starting in state $s_0$ is $\sum_{k=0}^{\infty}\gamma^{k} r_{t+k+1} = -1+\gamma2+\gamma^{2}3 \approx 3.92$. If we assume that the value of $s_0$ has never been updated before and is therefore $0$, and that we set $\alpha=0.5$, the result of one MC update for $s_0$ will be $\approx 1.96$. 

\begin{figure}[ht!]
	\centering
	\input{Images/Chapter06/mdp.tex}
